...
class Retriever:
    ...
    def embed_queries(self, args, queries):
        ...
        with torch.no_grad():
            for k, q in enumerate(queries):
                ...
                print("querys")
                print(q)
                batch_question.extend(q)
                # batch_question = [q['query'] for q in queries]
    ...
    def embed_queries_demo(self, queries):
        embeddings, batch_question = [], ["What is the historical background of the Jewish people?"]
        with torch.no_grad():
            for k, q in enumerate(queries):
...
def load_historical_data():
    # 这里假设历史数据存储在一个JSON文件中
    file_path = "../script_farren/input_data/test_input.json"
...
def main(args):
    ...
    # Iterate over queries and call search_document function
    query_results = []
    queries = [query.strip() for query in queries]
    results = retriever.search_document(queries, args.n_docs)
    for query, result in zip(queries, results):
        query_result = {"query": query, "results": result}
    ...
    if args.output_dir:
        ...
        with open(output_path, "w") as fout:
            for query_result in query_results:
                fout.write(json.dumps(query_result) + "\n")
