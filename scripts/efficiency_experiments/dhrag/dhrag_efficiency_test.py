...
class Config:
    def __init__(self, args):
        self.args = args
        self.OUTPUT_DIR = args.output_dir if args.output_dir else os.path.join("./output", "output")
        self.MONITOR_INTERVAL = args.monitor_interval
        self.ENABLE_GPU_MONITORING = args.enable_gpu_monitoring
        self.DETAILED_MONITORING = args.detailed_monitoring
...
def retrieve(args, paired_query, historical_database, performance_monitor=None):
    ...
    for idx, data in enumerate(paired_query):
        ...
        if debugger:
            end_time = time.time()
            debugger._record_operation_time("retrieval", end_time - start_time)
...
def parse_arguments():
    ...
    parser.add_argument(
        "--detailed_monitoring",
        action="store_true",
        help="Enable detailed performance monitoring with more metrics"
    )

    return parser.parse_args()

# 修改: generate_answer 函数（继续）
def generate_answer(args, integrated_results, paired_data, performance_monitor=None):
...
if __name__ == "__main__":
    ...
    debugger = PerformanceDebugger(
        log_interval=config.MONITOR_INTERVAL,
        detailed_monitoring=config.DETAILED_MONITORING  # 传递详细监控设置
    )
    ...
    try:
        ...
        with debugger.monitor_operation("init_distributed"):
        ...
        # 加载带有历史记录和查询的有效数据
        with debugger.monitor_operation("load_data"):
            paired_data = load_valid_data_with_history_and_query(input_file_path)
        
        # 初始化输出文件
        with debugger.monitor_operation("init_files"):
            initialize_output_files()
        
        # 初始化或加载历史数据库
        with debugger.monitor_operation("load_historical_db"):
        ...
        if paired_data:
            ...
            # 初始动态更新历史数据库
            with debugger.monitor_operation("update_historical_db"):
            ...
            # 执行BLEU评估
            with debugger.monitor_operation("bleu_evaluation"):
