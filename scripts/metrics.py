...
def qa_f1_score(prediction, ground_truth):
    if isinstance(ground_truth, list):
        # If ground_truth is a list, compute F1 for each and return the best
...
def normalize_answer(s):
    if isinstance(s, list):
        # If s is a list, normalize each element separately
        return [normalize_answer(item) for item in s]
