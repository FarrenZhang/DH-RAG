...
def cluster_historical_database(historical_database):
    if not historical_database:
        logging.warning("Historical database is empty. Returning empty list.")
        return []
        
    queries = [item['query'] for item in historical_database]
    vectorizer = TfidfVectorizer(max_features=1000)
    X = vectorizer.fit_transform(queries)
    
    n_clusters = min(10, len(queries))  # 可以根据需要调整聚类数量
    kmeans = KMeans(n_clusters=n_clusters, random_state=42)
    cluster_labels = kmeans.fit_predict(X)
    
    clustered_data = []
    with open(CLUSTERED_DATABASE_FILE, 'w') as f:
        for item, label in zip(historical_database, cluster_labels):
            entry = {
                'query': item['query'],
                'answer': item['answer'],
    ...
    for entry in clustered_data:
        ...
        if cluster_id not in clustering_output['clusters']:
            clustering_output['clusters'][cluster_id] = []
        clustering_output['clusters'][cluster_id].append(entry['query'])
    
    with open(os.path.join(OUTPUT_DIR, "historical_query_clustering_output.json"), 'w') as f:
        json.dump(clustering_output, f, ensure_ascii=False, indent=4)
    
    logging.info(f"聚类结果已保存到 {os.path.join(OUTPUT_DIR, 'historical_query_clustering_output.json')}")
...
def hierarchical_matching(query, historical_database):
    ...
    for cluster, cluster_similarity in top_clusters:
        ...
        for (historical_query, answer, timestamp, standard_answer), _ in cluster_items:
            try:
                ...
                fine_grained_matches.append(match_entry)
                cluster_entry['matches'].append(match_entry)
            except ValueError as e:
                logging.warning(f"Error calculating similarity for query '{historical_query[:50]}...': {str(e)}")
                continue
        hierarchical_structure['top_clusters'].append(cluster_entry)
    ...
    final_matches = sorted(sorted_matches, key=lambda x: x["combined_similarity"], reverse=True)[:5]
    
    hierarchical_structure = {
        'timestamp': datetime.now().strftime("%Y%m%d_%H%M%S"),
        'query': query,
        'top_clusters': hierarchical_structure['top_clusters'],
        'final_matches': final_matches
    }
    
    # 修改为追加模式
    try:    
        with open(HIERARCHICAL_MATCH_OUTPUT_FILE, 'a') as f:  # 使用全局变量
            json.dump(hierarchical_structure, f, ensure_ascii=False)
            f.write('\n')  # 添加换行符分隔每个结果
        logging.info(f"层次匹配结果已追加到 {HIERARCHICAL_MATCH_OUTPUT_FILE}")
    except Exception as e:
        logging.error(f"保存层次匹配结果时出错: {str(e)}")
...
def chain_of_thought_search(query, historical_database, max_steps=5, similarity_threshold=0.4):
    ...
    for step in range(max_steps):
        ...
        for item in historical_database:
            historical_query = item['query']
            historical_vector = model.encode(historical_query, convert_to_tensor=True).cpu().numpy()
            similarity = cosine_similarity_numpy(query_vector, historical_vector)
            logging.debug(f"Similarity between '{query}' and '{historical_query}': {similarity}")
            
            if similarity > best_similarity and historical_query not in visited:
                best_similarity = similarity
                best_match = item
        
        if best_match and best_similarity > similarity_threshold:
            logging.info(f"Best match query: {best_match['query']}, Similarity: {best_similarity}")
            results.append(best_match)
            chain_of_thought.append({
                "step": step + 1,
                "query": best_match['query'],
                "answer": best_match['answer'],
                "similarity": best_similarity  # 确保 similarity 是浮点数
            })
            visited.add(best_match['query'])
            # 更新查询向量为新的匹配查询向量（可选）
            query_vector = model.encode(f"{query} {best_match['query']}", convert_to_tensor=True).cpu().numpy()
        else:
            logging.info(f"No suitable match found in step {step + 1}. Exiting search.")
            break
    
    logging.info(f"Chain of thought search completed. Found {len(results)} results.")
    
    # 保存思维链结果，仅保存与思维链相关的信息
    try:
        with open(CHAIN_OF_THOUGHT_RESULTS_FILE, 'a', encoding='utf-8') as f:
            result = {
                "query": query,
            ...
            f.write('\n')  # 每个结果独占一行
        logging.info(f"Chain of thought results saved to {CHAIN_OF_THOUGHT_RESULTS_FILE}")
    except Exception as e:
...
def retrieve(args, paired_query, historical_database):
    ...
    for idx, data in enumerate(paired_query):
        ...
        integrated_results = integrate_results(static_results, hierarchical_matches, chain_of_thought_matches, query)
    ...
    if args.output_dir:
        try:
            with open(RETRIEVAL_RESULTS_FILE, "w", encoding='utf-8') as fout:
...
def update_historical_database(historical_database, new_query, answer, standard_answer):
    ...
    try:
        with open(HISTORICAL_DATABASE_FILE, 'a', encoding='utf-8') as f:
...
if __name__ == "__main__":
    ...
    if paired_data:
        ...
        for data in paired_data:
            if 'system' in data.query:  # 确保存在标准答案
                historical_database = update_historical_database(
                    historical_database, 
                    data.query['user'],  # 新的查询
                    data.query['system'],  # 答案（在这里使用标准答案作为初始答案）
                    data.query['system']  # 标准答案
                )
        
        # 对历史数据库进行聚类
        clustered_database = cluster_historical_database(historical_database)

        # 执行检索操作,获取集成结果(现在包含Chain of Thought搜索结果)
        integrated_results = retrieve(args, paired_data, clustered_database)
        ...
        for data, answer in zip(paired_data, gpt3_answers):
            historical_database = update_historical_database(
                historical_database, 
                data.query['user'],  # 新的查询
                answer['gpt3_answer'],  # GPT-3生成的答案
                data.query['system']  # 标准答案
            )
        
        # 执行BLEU评估
        average_bleu = perform_bleu_evaluation()
        logging.info(f"整体平均BLEU分数: {average_bleu:.4f}")
    else:
        # 如果无法加载有效数据,记录错误并退出程序
        logging.error("加载有效数据失败。正在退出程序。")
        sys.exit(1)
